...
function getCordInsertionString : String;
begin
...
begin
    ...
    result := result + vPlacentaString + ' with a '+ getCordInsertionString();
...
begin                                                                             
    ...
    begin        
     ...
     if not((cbLowLyingPlacentaYes1.checked) or (cbLowLyingPlacentaNo1.Checked) or (cbLowLyingPlacentaNotAssessed1.Checked)) then       
    begin        
      ShowMessage('Please enter yes/no/NA for lowlying placenta');
      result := False;     
    end;   
...
function getPlacentaLowLyingString : String;
begin
...
begin
    ...
    if cbPlacentaSite1.text <> '' then
        vPlacentaString := lowercase(cbPlacentaSite1.text);
    else
        vPlacentaString := '## '
...
begin
  if cbConsistentNIPTYes1.checked then
    ...
    result := getFirstName() + ' did not find out the gender from the NIPS result'  
end;
...
begin
    ...
    result := 'The fetal gender appears ' + getGenderStr()     
    result := result + ' and the patient is aware. '      
    result := result + getNipsGenderResult();  
    result := #13#10 + #13#10 + AddFullStop(result);                
 end;                                  
