// ******************* Utility Functions ************************************//
var
  GLocConclusion, GLeftLoc: String;
...
begin
  result := '';
  if cbPastSurgery.ItemIndex = (cbPastSurgery.Properties.Items.Count - 1) then
    result := edtOtherPastSurgery.Text
  else if cbPastSurgery.ItemIndex > 0 then
    result := cbPastSurgery.Text;
  if result <> '' then
    result := result + '. '
  else
    result := 'Nil. ';
...
{begin
  ...
  begin
    ...
    begin
      ...
      begin
        result := format(#13#10+ '%s',[GetMultiCystDescriptions(cbLeftOvaryCystType1.Selection, 2, 'L','')]);
      end;
    end
    else
    begin
      if (cbLeftOvaryCystType1.Selection = cbLeftOvaryCystType2.Selection) and (cbLeftOvaryCystType1.Selection = cbLeftOvaryCystType3.Selection)  then
      begin
        result := format(#13#10+ '%s',[GetMultiCystDescriptions(cbLeftOvaryCystType1.Selection, 3, 'L','')]);
      end
      else
      begin
        if (cbLeftOvaryCystType1.Selection = cbLeftOvaryCystType2.Selection) then
