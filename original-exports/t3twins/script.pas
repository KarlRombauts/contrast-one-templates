...
function GetPlacenta2InvertPosition: String;
begin
  result := Uppercase(trim(cbPlacentaSite2.text));
  if pos('ANTERIOR', UpperCase(result)) > 0 then
    result := StringReplace(result, 'ANTERIOR', 'POSTERIORLY')
  else
  if pos('POSTERIOR', UpperCase(result)) > 0 then
...
begin
 ...
 Function getPlacenta2Position : String
  if cbLowLyingPlacentaYes2.checked then
    ...
    else if cbLowLyingPlacentaNo2.checked then
    begin
        result := GetPlacenta2 + ' and not low lying.  ';
        GPlacentaWarning := False;
    end;
...
begin
  edtReferralIndicator1.Enabled := (ccbGestationType.States[cxccbReferralIndicators.Properties.Items.Count-1] = 1);
end; }
